...
interface SquareProp {
  ...
  setplayerBEats: React.Dispatch<React.SetStateAction<JSX.Element[]>>;
  setplayerAEats: React.Dispatch<React.SetStateAction<JSX.Element[]>>;
  playerAEats: JSX.Element[];
  playerBEats: JSX.Element[];
  sethighlightedBoxes: React.Dispatch<React.SetStateAction<number[][]>>;
  currentClickedPiece: any[];
  currentTurn: string;
  setcurrentTurn: React.Dispatch<React.SetStateAction<string>>;
  setcurrentClickedPiece: React.Dispatch<React.SetStateAction<any[]>>;
  setBoard: React.Dispatch<
    React.SetStateAction<(React.JSX.Element[] | string[])[]>
  >;
  board: (React.JSX.Element[] | string[])[];
  square: JSX.Element | string;
}

interface CheckMateCaseProp {
  kingId: string;
  newBoard: (React.JSX.Element[] | string[])[];
  targetKingPositionCol: number;
  targetKingPositionRow: number;
  highlightCheckPath: number[][];
}
...
const Square = ({
  ...
  const is2DArray = useCallback((array: any) => {
  ...
  const storeReactComponentInLocalStorage = useCallback(
    (
      ...
      if (is2DArray(array)) {
        serializableBoard = (array as (React.JSX.Element[] | string[])[]).map(
          (row) =>
            row.map((square) =>
              square ? (square as JSX.Element).props.id : ""
            )
        );
      } else {
  ...
  function saveBoardProgress(board: (React.JSX.Element[] | string[])[]) {
    storeReactComponentInLocalStorage(board, "board");
  }

  function savePlayerBProgress(playerBEats: React.JSX.Element[]) {
    storeReactComponentInLocalStorage(playerBEats, "playerBEats");
  }

  function saveCurrentTurn(currentTurn: string) {
    localStorage.setItem("currentTurn", currentTurn);
  }

  function savePlayerAProgress(playerAEats: React.JSX.Element[]) {
  ...
  const deepCloneArray = useCallback((arr: any): any => {
            ...
            if (isOpponentPiece) {
              ...
              const isAttackingKing = response.some(
                ([responseRow, responseCol]) => {
                  ...
                  if (
                    enemy !== "" &&
                    (enemy as JSX.Element).props.id === currentPlayerKingId
                  ) {
              ...
              const isAttackingKing = response.some(
                ([responseRow, responseCol]) => {
                  ...
                  if (
                    enemy !== "" &&
                    (enemy as JSX.Element).props.id === opponentPlayerKingId
                  ) {
                    kingPosition.push([responseRow, responseCol]);
                    return true;
                  }
    ...
    ({
      ...
      for (let x = 0; x < newBoard.length; x++) {
        for (let y = 0; y < newBoard[x].length; y++) {
          ...
          if (piece !== "") {
            const pieceId = (piece as JSX.Element).props.id;
  ...
  const [{ canDrop, isOver }, drop] = useDrop(
    {
      accept: "warrior",
      drop: (item: { col: number; row: number }, monitor) => {
        const { col, row } = item;
  ...
  return (
    <div
      className={`${css.square} ${dark ? css.dark : css.light} ${css.border} square-${
        index * 8 + id
      } ${isActive ? css.active : ""} ${shouldHighlight ? css.highlighted : ""} ${
